...
(function () {
  ...
  function areaChart(values, { w = 640, h = 180, color = "var(--indigo)", id = "ac" } = {}) {
    ...
    const grid = [0, 25, 50, 75, 100].map((g) =>
      `<line x1="${padL}" y1="${py(g)}" x2="${w - padR}" y2="${py(g)}" stroke="var(--grid)" stroke-width="1"/>
       <text x="${padL - 6}" y="${py(g) + 3}" text-anchor="end" font-size="9" fill="var(--muted)">${g}</text>`).join("");
  ...
  // 7-axis radar, values 0-100, labels Mon..Sun
  function radar(values, labels, { size = 230, color = "var(--teal)" } = {}) {
    const c = size / 2, R = size / 2 - 30, n = values.length;
    const pt = (i, r) => {
